...
def train(config: dict) -> float:
    ...
    with mlflow.start_run(run_name=f"{config['encoder']}-{config['dataset']}-s{config['seed']}") as run:
        ...
        # Grid calibration (corregido en la revision critica): only kan-bspline needs it. Uses a
        ...
        #
        # 50k rows, not a fixed batch count (verificado empiricamente): I6 and I12 reach ~690 standard
