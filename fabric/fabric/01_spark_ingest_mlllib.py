...
try:
    ...
    for _p in ["Files/models/mlllib_pipeline", "Tables/train", "Tables/val",
               "Tables/test", "Tables/_raw_ingested", "Tables/cat_index_maps"]:
        try:
...
def transform(df):
    return apply_indexers(apply_scaler(df))

train_t = transform(train_log)
val_t   = transform(val_log)
test_t  = transform(test_log)
