...
try:
    from kanrec.config import atlas_uri            # paquete instalado en el lakehouse
except ImportError:                                # fallback si el wheel aun no esta instalado
    def atlas_uri() -> str:
        import os
        try:
            import notebookutils
            secret = notebookutils.credentials.getSecret("kanrec-kv", "atlas-uri")
            if secret:
                return secret
        except Exception:
            pass
        ...
        if not uri:
            raise RuntimeError(
                "ATLAS_URI no configurada. Anade el secreto 'atlas-uri' al Key Vault "
                "'kanrec-kv' o define la variable de entorno ATLAS_URI."
            )
        return uri
